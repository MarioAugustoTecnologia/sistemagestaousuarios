 ...
 */
async function sendTo(phoneOrContact, message, archive) {
  ...
  if (typeof phoneOrContact === "object") {
    ...
    archive = generateCustomArchive(phoneOrContact, archive);
  }
  try {
    process.stdout.write("Sending Message...\r");
    await page.goto(
      `https://web.whatsapp.com/send?phone=${phone}&text=${encodeURIComponent(message)}&file=${encodeURIComponent(archive)}`
    );
 ...
 */
async function send(phoneOrContacts, message, archive) {
  for (let phoneOrContact of phoneOrContacts) {
    await sendTo(phoneOrContact, message, archive);
  }
}
 ...
 * @param {string} messagePrototype message to send to every phone number
 * @param {File} file
...
function generateCustomArchive(contact, file) {
  let archive = file;
  for (let property in contact) {
